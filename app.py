...
def BuscarSecao():
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT id, nome FROM SECAO Where ativa = true ' + MontaPredicadoBuscaSecao(request))
    secoes =  cursor.fetchall()
    cursor.close()
...
def getBuscaProdutos():
    ...
    """ 
    cursor =  conn.cursor()
    cursor.execute('SELECT id, nome, preco, idSecao FROM PRODUTOS Where ativa = true ' + MontaPredicadoBuscaProduto(request))
    produtos =  cursor.fetchall()
